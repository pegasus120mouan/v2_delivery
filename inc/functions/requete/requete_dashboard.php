...
$nombreColisLivre = $conn->prepare('SELECT COUNT(*) AS nbColis FROM commandes where date_livraison=CURRENT_DATE() and statut="Livré"');
...
SELECT SUM(commandes.cout_reel) AS montant_reel_colis_uniko FROM commandes JOIN utilisateurs ON commandes.utilisateur_id = utilisateurs.id JOIN boutiques ON utilisateurs.boutique_id = boutiques.id WHERE commandes.livraison = CURRENT_DATE() AND commandes.statut = 'Livré' AND boutiques.nom = 'Uniko Perfume'");
...
$sqlpoints="SELECT
    ...
    -- Montants totaux pour les colis livrés aujourd'hui
    SUM(CASE WHEN c.statut = 'Livré' AND c.date_livraison = CURRENT_DATE() THEN c.cout_global ELSE 0 END) AS total_amount,
    SUM(CASE WHEN c.statut = 'Livré' AND c.date_livraison = CURRENT_DATE() THEN c.cout_reel ELSE 0 END) AS total_cout_reel,
    SUM(CASE WHEN c.statut = 'Livré' AND c.date_livraison = CURRENT_DATE() THEN c.cout_livraison ELSE 0 END) AS total_cout_livraison,
    -- Total des colis (livrés aujourd'hui uniquement)
    COUNT(CASE WHEN c.date_livraison = CURRENT_DATE() THEN 1 ELSE NULL END) AS total_orders,
    -- Total des colis livrés aujourd'hui
    SUM(CASE WHEN c.statut = 'Livré' AND c.date_livraison = CURRENT_DATE() THEN 1 ELSE 0 END) AS total_delivered_orders,
    -- Total des colis non livrés (date_reception = aujourd'hui et date_livraison IS NULL)
    SUM(CASE WHEN c.date_reception = CURRENT_DATE() AND c.date_livraison IS NULL THEN 1 ELSE 0 END) AS total_undelivered_orders
FROM
    commandes c
JOIN utilisateurs u ON c.utilisateur_id = u.id
JOIN boutiques b ON u.boutique_id = b.id
WHERE
    -- Inclure les colis livrés aujourd'hui ou non livrés enregistrés aujourd'hui
    (c.date_livraison = CURRENT_DATE() OR (c.date_reception = CURRENT_DATE() AND c.date_livraison IS NULL))
    AND u.role = 'clients'
GROUP BY
    u.id, b.nom";
...
WHERE DATE(commandes.date_livraison) = CURDATE() AND commandes.statut = 'livré'
...
FROM
    commandes
WHERE
    DATE(date_livraison) = CURRENT_DATE
    AND statut = 'Livré'
GROUP BY
    livreur_id
