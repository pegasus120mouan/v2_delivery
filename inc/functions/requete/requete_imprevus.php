...
$stmt = $conn->prepare(
    ...
    imprevu.montant, 
    imprevu.motif, 
    imprevu.date_contraction,
   concat(utilisateurs.nom,' ',utilisateurs.prenoms) AS nom_livreur 
FROM 
    imprevu
INNER JOIN 
    utilisateurs ON imprevu.livreur_id = utilisateurs.id;");
...
/*$getSommeImprevus = "SELECT SUM(montant) AS somme_totale_imprevus
FROM imprevu";
$getSommeImprevuStmt = $conn->query($getSommeImprevus); */
