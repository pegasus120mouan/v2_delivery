...
$montantClient = $conn->prepare('SELECT SUM(cout_reel) AS reel_somme FROM commandes where date_commande=CURRENT_DATE()-1 and statut="Livré"');
...
$montantLivraison = $conn->prepare('SELECT SUM(recette) AS somme_livraison FROM points_livreurs where date_commande=CURRENT_DATE()-1');
...
$montantDepenses = $conn->prepare('SELECT SUM(depense) AS somme_depenses FROM points_livreurs where date_commande=CURRENT_DATE()-1');
...
$nombreColisLivrehier = $conn->prepare('SELECT COUNT(*) AS nbColishier FROM commandes where date_commande=CURRENT_DATE()-1 and statut="Livré"');
...
/*$montantGlobalLass_hier = $conn->prepare("SELECT sum(cout_reel) from utilisateurs join commandes on utilisateurs.id=commandes.livreur_id where date_commande=CURRENT_DATE()-2 and utilisateurs.nom='Kone' and utilisateurs.prenoms='Lassina' and commandes.statut='Livré'");
...
/*$montantDepensesLass_hier = $conn->prepare('SELECT SUM(depenses) AS depenses_hier_lass FROM points_livreurs where date_commande=CURRENT_DATE()-1 and prenoms_livreur="Lassina"');
...
// Point Fofana
/*$montantGlobalFof_hier = $conn->prepare('SELECT SUM(cout_global) AS montant_global_colis_Fofana_hier FROM commandes where date_commande=CURRENT_DATE()-1 and statut="Livré" and livreur="Fofana"');
...
$montantDepensesFof_hier = $conn->prepare('SELECT SUM(depenses) AS depenses_hier_fof FROM points_livreurs where date_commande=CURRENT_DATE()-1 and prenoms_livreur="Fofana"');
